...
function getTimestampOffset(json) {
  if ("async-attributes" in json) {
    const asyncAttributes = json["async-attributes"];

    // v1 gasless 
    if ("default_utc_offset" in asyncAttributes) {
      return asyncAttributes.default_utc_offset;
    } else if ("timezone" in asyncAttributes) {
      // v2 gasless
      return asyncAttributes.timezone.default_utc_offset;
    }
  }
...
async function renderLocal(layoutFilePath, tokenFilePath, options) {
  ...
  const timestampOffset = getTimestampOffset(json);
