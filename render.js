...
/* 
 * Options:
 * blockNum - (optional) render the art as of this block number 
 * stampDebug - whether or not to stamp debug info on the rendered image
 * tokenURI - (optional) tokenURI to override rather than call onchain (use for debugging locally)
 */
async function process(provider, tokenAddress, tokenId, options) {	
	var network = await provider.getNetwork();

	try {
		var returnData = await onNetworkLoaded(provider, tokenAddress, tokenId, options);
...
async function onNetworkLoaded(provider, tokenAddress, tokenId, options) {
	let contract = new ethers.Contract(tokenAddress, CONTRACT_ABI, provider);

	var tokenURI = options.tokenURI;
	
	// if no token URI was specified in options then retrieve it from the chain
	if ((tokenURI == undefined) || (tokenURI == null) || (tokenURI.length == 0)) {
	...
	// now that we got the master token URI, retrieve the layout file
	...
	// render the image
	var finalImage = await renderer.render(contract, layout, options, tokenId);

	// stamp the metadata
	if (options.stampDebug) {
		await stampBlockNumber(finalImage, options.blockNum);
	}
