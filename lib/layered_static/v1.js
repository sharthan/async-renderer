...
async function readIntProperty(object, key, label, tokens, masterTokenId) {
  ...
  if (typeof value === "object") {    
    ...
    if (PROPERTY_TOKEN in object[key]) {
      ...
      timestamp += object[key][PROPERTY_TIME]["default_utc_offset"];

      var date = new Date(timestamp * 1000);

      var segment = object[key][PROPERTY_TIME]["segment"];

      if (segment == "DAY_OF_YEAR") {        
        ...
        var maxBoundInclusive = object[key][PROPERTY_TIME]["max_bound_inclusive"];
        // modulo to keep the value within the max bound inclusive
        value = dayOfYear % (maxBoundInclusive + 1);
        console.log(`    ${label} = ${value} (Time=${timestamp}, DayOfYear=${dayOfYear}, MaxBoundInclusive=${maxBoundInclusive})`);
      }
    }
  } else {
